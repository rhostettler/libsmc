...
L = 5;         % Number of iterations (if applicable)
...
sample_sp = @(model, y, x, theta, L) sample_gaussian_sp(model, y, x, theta, f, @(x, theta) Q, g, @(x, theta) R, L, Xi, wm, wc);
par_sp = struct( ...
    'sample', @(model, y, x, theta) sample_sp(model, y, x, theta, L), ...
    'calculate_incremental_weights', @calculate_incremental_weights_generic ... %, ...
    ... % 'resample', @(lw) resample_ess(lw, par_resample) ...
);
