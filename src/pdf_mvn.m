function py = pdf_mvn(dx, m, P, fast)
...
%}
    ...
    if ~isa(m, 'function_handle') && ~isa(P, 'function_handle')
        LP = chol(P).';
        py = struct( ...
            'rand', @(x, theta) m + LP*randn(size(x)), ...
            'logpdf', @(y, x, theta) logmvnpdf(y.', m.', P).', ...
            'fast', fast, ...
            'kappa', @(x, theta) (2*pi)^(-dx/2)*det(P).^(-1/2) ...
        );
    elseif ~isa(m, 'function_handle') && isa(P, 'function_handle')
        py = struct( ...
            'rand', @(x, theta) m + chol(P(x, tehta)).'*randn(size(x)), ...
            'logpdf', @(y, x, theta) logmvnpdf(y.', m.', P(x, theta)).', ...
            'fast', fast, ...
            'kappa', @(x, theta) (2*pi)^(-dx/2)*det(P(x, theta)).^(-1/2) ...
        );
    elseif isa(m, 'function_handle') && ~isa(P, 'function_handle')
        LP = chol(P).';
        py = struct( ...
            'rand', @(x, theta) m(x, theta) + LP*randn(size(x)), ...
    ...
    elseif isa(m, 'function_handle') && isa(P, 'function_handle')
        py = struct( ...
            'rand', @(x, theta) m(x, theta) + chol(P(x, theta)).'*randn(size(x)), ...
